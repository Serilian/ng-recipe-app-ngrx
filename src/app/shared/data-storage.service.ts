...
export class DataStorageService {
  ...
  storeRecipes() {
    ...
    //   // headers: headers
    // });
    const req = new HttpRequest('PUT', 'https://ng-recipe-book-9f137.firebaseio.com/recipeState.json', this.recipeService.getRecipes(), {reportProgress: true});
    return this.httpClient.request(req);
  }

  getRecipes() {
    // this.httpClient.get<Recipe[]>('https://ng-recipe-book-3adbb.firebaseio.com/recipes.json?auth=' + token)
    this.httpClient.get<Recipe[]>('https://ng-recipe-book-9f137.firebaseio.com/recipeState.json', {
      observe: 'body',
      responseType: 'json'
    })
      .map(
